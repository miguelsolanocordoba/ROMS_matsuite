...
% Read NCOM grid
%
load ncom_bathymetry.mat
h_ncom=h; clear h

% Interpolate NCOM grid to ROMS grid
[x1,x2]=meshgrid(lon_ncom,lat_ncom); 
[y1,y2]=meshgrid(lonr,latr); 
h_ncom_interp=interp2(x1,x2,h_ncom',y1,y2); 
...
xr1=linspace(lonr(1),lonr(end),numel(lonr)); 
xr2=linspace(latr(1),latr(end),numel(latr)); 
...
% Nudgin to NCOM bathymetry
h=alpha.*h_ncom_interp+(1-alpha).*h_roms; 
pcolor(abs(h-h_roms)); shading interp; colorbar
...
%!convert -density 600 bathy.ps bathy.png
